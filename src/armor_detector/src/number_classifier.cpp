...
namespace rm_auto_aim
{
  NumberClassifier::NumberClassifier(
    ...
    const std::vector<std::string> & ignore_classes):threshold(thre),ignore_classes_(ignore_classes)
    {
      this->net_=cv::dnn::readNetFromONNX(model_path); //读取模型

      std:: ifstream label_file(label_path);     // 读取分类标签
      std::string line;
      ...
      //   std::cerr << "Error: class_names_ is empty. Check the label file: " << label_path << std::endl;
      //   // 可以选择进行一些错误处理，例如抛出异常或者终止程序
      //   throw std::runtime_error("class_names_ is empty");
      // }
    }
  // 提取数字图像
  void NumberClassifier::extractNumbers(const cv::Mat &src,std::vector<Armor> &armors)
  {
    ...
    for(auto & armor : armors)
    {
      ...
      auto rotation_matrix =cv::getPerspectiveTransform(light_vertices,target_vertices);
      cv::Mat number_image;
      cv::warpPerspective(src,number_image,rotation_matrix,cv::Point(warp_width,warp_height));
      // Get ROI
